...
if (dataset == 'UrbanTracker'):
    ...
    gt_st_frame =1
    path, dirs, files = next(os.walk('/home/huooi/HL_Dataset/UA_Detrac_Detection/Dataset/Insight-MVT_Annotation_' + 'Train' + '/' + seq_name + '/'))
...
weights = torch.load("osnet_ibn_x1_0_msmt17_combineall_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip_jitter.pth")
features_length = 512
...
while (True):
    ...
    if (dataset == 'UrbanTracker'):
        ...
        imot_src = '/home/huooi/HL_Dataset/Urbantracker_bgs/DIL_IMOT_' + seq_name + '_PAWCS/' + str(frame_num).zfill(8) + '.png'  # imot based on PAWCS
    elif (dataset == 'UADetrac_Train'):
        imot_src = '/home/huooi/HL_Dataset/uadetrac-bgs/' + seq_name + '/With_mask/PAWCSImot/' + str(frame_num).zfill(8) + '.png'  # imot based on PAWCS
    elif (dataset == 'UADetrac_Test'):
        imot_src = '/home/huooi/HL_Results/IMOT_Subsense/UADetrac/test/' + seq_name + '/'+ str(frame_num).zfill(
    ...
    if (ret_imot):
        ...
        contours, hier = cv2.findContours(imot_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # height, width = image_np.shape[:2]
        for c in contours:
    ...
    for i in range(len(input_labelled)):
        img = image_np[input_labelled[i][0][0]:input_labelled[i][0][1], input_labelled[i][0][2]:input_labelled[i][0][3]]
    ...
    for i in range(len(input_labelled)):
        ...
        input_comb.append([input_labelled[i][0], input_labelled[i][1], input_labelled[i][2], input_labelled[i][3], query_features[i].detach().cpu().numpy()])
    ...
    # feature_index = [1,3,4] #ICIAR 2019 feature combination
